...
def loadTextFeatures():
    ...
    text_features_files = [
        f for f in listdir("./{}/".format(text_module_name))
        if (isfile(join("./{}".format(text_module_name), f)) and
    ...
    if text_features_files:
        ...
        features = pickle.load(
            open("./{}/{}".format(text_module_name, text_features_files[0]),
                 'rb'))
        features = [x for sublist in features
                    for x in sublist]  # Flat into 1D List
        print '\n===' + turquoise + ' FEATURES DATA SET LENGTH: {} '.format(
            len(features)) + normal + '==='
...
def map_readability(entry):
    ...
    from 1 to 6, where 1 - low complexity and 6 - greatest.
    Output value is used for music composition
    """
    if entry >= 13:
        return 6
    if entry >= 11:
...
def map_lex_density(entry):
    ...
    from 1 to 4, where 1 - low density and 4 - greatest.
    Output value is used for music composition
    """
    if entry >= 75:
        return 4
...
def map_text_parameters(group_size=5, visual=False):
    ...
    lex_score = list([x[1][0]
                      for x in features])  # List of all lexical density scores
    read_score = list([x[1][1]
                       for x in features])  # List of all readability scores
    ...
    pickle.dump(sent_score_mapped,
                open('./{}/{}'.format(module_name, 'sentiment_mapped.p'),
                     "wb"))
    pickle.dump(lex_score_mapped,
                open('./{}/{}'.format(module_name, 'lexical_mapped.p'), "wb"))
    pickle.dump(read_score_mapped,
                open('./{}/{}'.format(module_name, 'readability_mapped.p'),
                     "wb"))
    ...
    if visual:
        plot_features(
            ...
            lex_score_median,
            read_score_median,
            label='median value')  # medians visualisation
        plot_features(
            sent_score_mapped,
            lex_score_mapped,
            read_score_mapped,
            label='mapped value')  # mapped features visualisation
